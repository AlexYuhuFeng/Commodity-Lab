...
def _fetch_fut_basic_via_http(token: str, exchange: str) -> pd.DataFrame:
    ...
    try:
        # Prefer runtime proxy settings first (enterprise env compatibility).
        resp = requests.post(_TUSHARE_PRO_URL, json=payload, timeout=20)
    except requests.exceptions.ProxyError:
        # If proxy explicitly blocks Tushare, retry once with direct connection.
        # This helps local/dev environments where proxy vars are present but not needed.
        direct = Session()
        direct.trust_env = False
        resp = direct.post(_TUSHARE_PRO_URL, json=payload, timeout=20)
