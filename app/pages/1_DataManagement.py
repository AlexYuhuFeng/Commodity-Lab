...
with search_tab:
    ...
    ts_ok, ts_reason = tushare_status()
    if source_mode in ("all", "tushare") and not ts_ok:
        if ts_reason == "missing_token":
            st.info(l("Tushare token not configured. Set env `TUSHARE_TOKEN` to enable it.", "未配置 Tushare token。请设置环境变量 `TUSHARE_TOKEN`。"))
        elif ts_reason == "missing_package":
            st.warning(l("Tushare package is not installed in current runtime.", "当前运行环境未安装 tushare 包。"))
