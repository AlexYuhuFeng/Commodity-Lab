...
from core.tushare_provider import search_tushare, tushare_status
...
with search_tab:
    ...
    default_token = st.session_state.get("tushare_token", "")
    token_input = st.text_input(
        l("Tushare Token (optional)", "Tushare Token（可选）"),
        value=default_token,
        type="password",
        help=l("Input token from UI; not written to codebase.", "在前端输入 token，不写入代码。"),
    )
    st.session_state["tushare_token"] = token_input

    ts_ok, ts_reason = tushare_status(token_input)
    if source_mode in ("all", "tushare") and not ts_ok:
        if ts_reason == "missing_token":
            st.info(l("Tushare token not configured. Please input token above.", "未配置 Tushare token，请在上方输入。"))
        elif ts_reason == "invalid_token":
            st.error(l("Tushare token is invalid. Please verify your token.", "Tushare token 无效，请检查后重试。"))
        elif ts_reason == "network_error":
            st.warning(l("Cannot connect to Tushare API from current network.", "当前网络无法连接 Tushare API。"))
        elif ts_reason == "api_error":
            st.warning(l("Tushare API returned an error.", "Tushare API 返回错误。"))
    ...
    if query:
        ...
        if source_mode in ("all", "tushare"):
            try:
                ts_rows = search_tushare(query, max_results=max_results, token=token_input)
            except Exception as e:
