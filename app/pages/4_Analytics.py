...
transform_df = con.execute("SELECT DISTINCT derived_ticker FROM transforms WHERE enabled = TRUE ORDER BY 1").df()
transform_tickers = transform_df["derived_ticker"].tolist() if not transform_df.empty else []
all_tickers = sorted(set(raw_tickers + derived_tickers + transform_tickers))
...
def load_series(ticker: str) -> pd.DataFrame:
    out = _load_series_cached(ticker)
    if not out.empty:
        return out

    # If the series is transform-defined but derived_daily is stale/empty, try compute once.
    tf = con.execute("SELECT transform_id FROM transforms WHERE derived_ticker = ? LIMIT 1", [ticker]).fetchone()
    if tf:
        try:
            recompute_transform(con, tf[0])
            _load_series_cached.clear()
        except Exception:
            pass
...
with spread_tab:
    ...
    if merged.empty:
        st.warning(l("No overlapping dates between selected series.", "所选序列没有重叠日期。"))
        st.stop()
    ...
    if mode == "L-R":
        ...
        merged["spread"] = merged["left_norm"] / merged["right_norm"].replace(0, pd.NA)
    else:
        merged["spread"] = (merged["left_norm"] - merged["right_norm"]) / merged["right_norm"].replace(0, pd.NA)

    merged = merged.dropna(subset=["spread"]) 
    if merged.empty:
        st.warning(l("Spread result is empty after formula evaluation.", "按公式计算后价差为空。"))
        st.stop()
